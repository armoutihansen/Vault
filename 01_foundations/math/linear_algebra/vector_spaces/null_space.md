...
\mathrm{Ax} = 0.
$$
$\mathrm{Null(A)}$ is a [[01_foundations/math/linear_algebra/vector_spaces/vector_space|vector subspace]]: if $\mathrm{x}, \mathrm{y} \in \mathrm{Null(A)}$, then $a\mathrm{x} + b\mathrm{y} \in \mathrm{Null(A)}$. For an $m \times n$ matrix $\mathrm{A}$, the null space is a subspace of $\mathbb{R}^n$.
