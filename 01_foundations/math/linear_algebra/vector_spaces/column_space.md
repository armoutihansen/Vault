...
The _column space_ of a matrix $\mathrm{A}$, denoted $\mathrm{Col(A)}$, is the [[01_foundations/math/linear_algebra/vector_spaces/span_basis_dimension|span]] of the columns of $\mathrm{A}$. For an $m \times n$ matrix, $\mathrm{Col(A)}$ is a subspace of $\mathbb{R}^m$.
