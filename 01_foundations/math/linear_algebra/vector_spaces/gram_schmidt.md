...
Given any [[01_foundations/math/linear_algebra/vector_spaces/span_basis_dimension|basis]] $\{\mathrm{v}_1, \dots, \mathrm{v}_n\}$ for an $n$-dimensional [[01_foundations/math/linear_algebra/vector_spaces/vector_space|vector space]], the Gram-Schmidt process produces an orthonormal basis $\{\hat{\mathrm{u}}_1, \dots, \hat{\mathrm{u}}_n\}$ for the same space such that $\operatorname{span}\{\hat{\mathrm{u}}_1, \dots, \hat{\mathrm{u}}_k\} = \operatorname{span}\{\mathrm{v}_1, \dots, \mathrm{v}_k\}$ for all $k$.
